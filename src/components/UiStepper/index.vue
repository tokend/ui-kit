  ...
  <div class="ui-stepper">
    <template v-for="(title, i) in titles">
      <div
        :key="i"
        :class="{
          'ui-stepper__step': true,
          'ui-stepper__step--active': slotNames[i] === current,
        }"
      >
        <button
          ...
          <p class="ui-stepper__title">
            <span class="ui-stepper__circle">
              <template v-if="slotNames[i] < current">
                <i class="ui-stepper__completed-icon mdi mdi-check" />
              </template>
              <template v-else>
                {{ i+1 }}
              </template>
